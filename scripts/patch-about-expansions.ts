/**
 * Re-sync all About keyword expansions (fixes stale labels, double periods).
 *
 * Run from frontend/:
 *   sanity exec scripts/patch-about-expansions.ts --with-user-token
 */
...
async function main() {
  ...
  for (const keyword of Object.keys(ABOUT_EXPANSIONS)) {
    console.log(`  · ${keyword}`);
  }
}
