/**
 * OC Links §05 My Approach / Design Process (Figma 4003:105317).
 * Cream band #e3e3db, steel-blue accordion #668fbc, left copy #231e1e, panel text black.
 * Accordion: scripts/data/caseStudyCollabCopy.json → oc-links.approach
 * No collab blurb — keep existing sideBody (ignore Figma lorem).
 *
 * Run from frontend/:
 *   npx sanity exec scripts/patch-oc-links-approach.ts --with-user-token -- --dry
 *   npx sanity exec scripts/patch-oc-links-approach.ts --with-user-token
 *   npx sanity exec scripts/patch-oc-links-approach.ts --with-user-token -- --colors-only
 */
...
/** Figma 4003:105317 band */
const BAND_BG = "#e3e3db";
/** Figma 4003:105321 accordion panel */
const PANEL_BG = "#668fbc";
/** Figma 4003:105318 left copy */
const TEXT_COLOR = "#231e1e";
/** Figma 4003:105323–105335 panel copy */
const PANEL_TEXT = "#000000";

const approach = collab["oc-links" as keyof typeof collab] as {
  approach?: {
    blurb?: string;
...
async function main() {
  console.log(
    `patch-oc-links-approach (${DRY ? "dry" : COLORS_ONLY ? "colors-only" : "live"})`,
  );
