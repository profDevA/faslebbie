/**
 * OC Resource Navigator §03 Problem Context / What I Brought
 * (Figma desktop `4151:56976` / mobile `4151:59337`).
 *
 * Existing `problemContextSection` — black centred band. Collab copy.
 * Ignore Figma lorem and leftover Coral Health mobile prose.
 *
 * Run from frontend/:
 *   npx sanity exec scripts/patch-oc-navigator-problem-context.ts --with-user-token -- --dry
 *   npx sanity exec scripts/patch-oc-navigator-problem-context.ts --with-user-token
 *   npx sanity exec scripts/patch-oc-navigator-problem-context.ts --with-user-token -- --appearance-only
 */
...
async function main() {
  console.log(
    `patch-oc-navigator-problem-context (${DRY ? "dry" : "live"}${APPEARANCE_ONLY ? ", appearance-only" : ""})`,
  );
  ...
  console.log("✓ OC Resource Navigator Problem Context patched");
}
