...
| OC Links §04 View More popup — 12-tile flow grid | `patch-oc-links-core-experience-popup.ts` (Figma `4151:34943`; 12 plates; PNG: `core-flow/modal/01–14-*` minus crisis-check + resources; popup navy `#0A2A58`, tiles `#668FBC`). Single tab; all 12 visible. Intro from collab (ignore Figma exhibition leftover). **`--appearance-only`**, **`--trim-to-12`**. **Do not re-run full patch after manual uploads.** |
