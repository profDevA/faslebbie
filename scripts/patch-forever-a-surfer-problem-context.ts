/**
 * Forever a Surfer §03 Problem Context / What I Brought
 * (Figma `4162:23111` black band `4162:23160` — ignore Coral lorem).
 *
 * Copy: live WP / case-studies.generated.ts — heading **Campaign Background**,
 * What I Brought = three WP brought items.
 *
 * Run from frontend/:
 *   npx sanity exec scripts/patch-forever-a-surfer-problem-context.ts --with-user-token -- --dry
 *   npx sanity exec scripts/patch-forever-a-surfer-problem-context.ts --with-user-token
 *   npx sanity exec scripts/patch-forever-a-surfer-problem-context.ts --with-user-token -- --appearance-only
 */
...
async function main() {
  console.log(
    `patch-forever-a-surfer-problem-context (${DRY ? "dry" : "live"}${APPEARANCE_ONLY ? ", appearance-only" : ""})`,
  );
  ...
  console.log("✓ Forever a Surfer Problem Context patched");
}
