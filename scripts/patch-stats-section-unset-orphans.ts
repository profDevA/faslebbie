/** Remove legacy orphan keys on statsSection (caption, cta*, video*). */
import { getCliClient } from "sanity/cli";

const client = getCliClient({ apiVersion: "2025-01-01" });

const ORPHANS = ["caption", "ctaLabel", "ctaUrl", "videoFile", "videoUrl"] as const;

type Row = { _id: string; sections: Record<string, unknown>[] };

async function main() {
  const docs = await client.fetch<Row[]>(
    `*[_type == "caseStudy"]{ _id, sections[]{ _type, caption, ctaLabel, ctaUrl, videoFile, videoUrl } }`,
  );
