/**
 * Forever a Surfer §11 Reflection + Next Steps (Figma `4162:23711`).
 * Dark band #171717. Copy: live WP / case-studies.generated.ts (ignore Figma lorem).
 *
 * Run from frontend/:
 *   npx sanity exec scripts/patch-forever-a-surfer-reflection.ts --with-user-token -- --dry
 *   npx sanity exec scripts/patch-forever-a-surfer-reflection.ts --with-user-token
 *   npx sanity exec scripts/patch-forever-a-surfer-reflection.ts --with-user-token -- --appearance-only
 */
...
async function main() {
  console.log(
    `patch-forever-a-surfer-reflection (${DRY ? "dry" : APPEARANCE_ONLY ? "appearance-only" : "live"})`,
  );
