/**
 * OC Links §01 Hero + §02 Overview (Figma 4003:104522).
 *
 * Figma: desktop hero `4003:104524` (caption cropped — site overlays collab copy),
 * overview side `4004:113275` (navigator photo + login). No mobile hero frame yet.
 *
 *   public/work/oc-links/01-hero-desktop.png
 *   public/work/oc-links/03-overview-side.png
 *
 * Copy: scripts/data/caseStudyCollabCopy.json → oc-links.hero / oc-links.overview
 *
 * Run from frontend/:
 *   npx sanity exec scripts/patch-oc-links-hero-overview.ts --with-user-token -- --dry
 *   npx sanity exec scripts/patch-oc-links-hero-overview.ts --with-user-token
 *   npx sanity exec scripts/patch-oc-links-hero-overview.ts --with-user-token -- --copy-only
 *   npx sanity exec scripts/patch-oc-links-hero-overview.ts --with-user-token -- --hero-only
 *   npx sanity exec scripts/patch-oc-links-hero-overview.ts --with-user-token -- --overview-only
 */
...
const ASSET_DIR = join(process.cwd(), "public/work/oc-links");
const FILES = {
...
const copy = collab["oc-links" as keyof typeof collab] as {
  hero: {
    projectName: string;
    statement: string;
    from: string;
...
async function patchDoc(
  ...
  const { heroIdx, overviewIdx, heroImage, overviewImage } = opts;
...
async function main() {
  console.log(`patch-oc-links-hero-overview (${DRY ? "dry" : "live"})`);
  ...
  const opts = { heroIdx, overviewIdx, heroImage, overviewImage };
  ...
  console.log("✓ OC Links hero + overview patched");
}
