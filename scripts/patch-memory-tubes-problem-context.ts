/**
 * Memory Tubes §03 Problem Context / What I Brought
 * (Figma desktop `4152:101624` / mobile `4152:103447` band).
 *
 * Existing `problemContextSection` — black centred band. Collab copy.
 * Ignore Figma lorem and leftover template headings.
 *
 * Run from frontend/:
 *   npx sanity exec scripts/patch-memory-tubes-problem-context.ts --with-user-token -- --dry
 *   npx sanity exec scripts/patch-memory-tubes-problem-context.ts --with-user-token
 *   npx sanity exec scripts/patch-memory-tubes-problem-context.ts --with-user-token -- --appearance-only
 */
...
async function main() {
  console.log(
    `patch-memory-tubes-problem-context (${DRY ? "dry" : "live"}${APPEARANCE_ONLY ? ", appearance-only" : ""})`,
  );
  ...
  console.log("✓ Memory Tubes Problem Context patched");
}
